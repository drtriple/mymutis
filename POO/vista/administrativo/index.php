        ...
        if(empty($_REQUEST['name'])!=true && preg_match("/^[a-zñA-ZÑ-áéíóúÁÉÍÓÚ ,.'-]+$/i", $_REQUEST['name']) && strlen($_REQUEST['name']) > 1){
            if(empty($_REQUEST['apellidos'])!=true && preg_match("/^[a-zñA-ZÑ-áéíóúÁÉÍÓÚ ,.'-]+$/i", $_REQUEST['apellidos']) && strlen($_REQUEST['apellidos']) > 1){
                if(empty($_REQUEST['email'])!=true && preg_match("/^[a-zA-Z0-9\._-]+@[a-zA-Z0-9-]{2,}[.][a-zA-Z]{2,4}$/", $_REQUEST['email']) ){   
                    ...
                    switch ($_REQUEST['rol']) {
                        case 1:
                            if(empty($_REQUEST['emailAcudiente'])!=true && preg_match("/^[a-zA-Z0-9\._-]+@[a-zA-Z0-9-]{2,}[.][a-zA-Z]{2,4}$/", $_REQUEST['emailAcudiente']) && $_REQUEST['emailAcudiente'] != $_REQUEST['email']){
                                
                                //crear Usuario
                                 //contrasena
                                    $pass = $_REQUEST['user']."mymutis";
                                    //1025881144Cristian Andres
            ...
            <div class="row">
                    ...
                    <div class="col-lg-12 col-md-12">
                        <div class="card card-body mailbox">
                            ...
                            <h3 class="card-title">Historial de Login</h3>
                          
                            <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modalEliminarHistorial">Eliminar los últimos 10 inicios de sesión</button>

                        <!-- Modal -->
                        <div class="modal fade" id="modalEliminarHistorial" tabindex="-1" aria-labelledby="modalEliminarHistorialLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalEliminarHistorialLabel">ELIMINAR</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                ¿Estás seguro que deseas eliminar los últimos 10 inicios de sesión? <br> (Esto es una acción permanente)
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                <a class="btn btn-danger" href="../../../POO/vista/administrativo/index.php?btnBorrarHistorial">Eliminar</a>
                            </div>
                            </div>
                        </div>
                        </div>
                            ...
                            <?php	
                            foreach($listarHistoria as $historiallogin){ 
                            ...
                            ?>
                                ...
                                <a href="#">
                                    ...
                                    <div class="mail-contnet">
                                        <h6 class="text-dark font-medium mb-0"><?php echo $historiallogin['docUsuario']; ?></h6> <span class="mail-desc"><?php echo $historiallogin['fechaIngreso']; ?></span>
                                        <span class="time"><?php echo $historiallogin['horaIngreso']; ?> <?php echo $historiallogin['horario']; ?></span>
                                    </div>
                                </a>
                                <?php	
                           }
